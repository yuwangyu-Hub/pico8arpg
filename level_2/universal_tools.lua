...
function check_wall_iswalk(v,w,h)--检测物体(角色、箱子)是否靠近墙壁（1-8分别对应墙靠近玩家的位置，0不靠墙）
	...
	--检测该点是否在图块上
	--八个点分别为上下左右四个侧面的两个端点。
	local x1,y1=flr((v.x-1)/8),flr((v.y)/8) 
	local x2,y2=flr((v.x-1)/8),flr((v.y+h-1)/8)
	local x3,y3=flr((v.x)/8),flr((v.y+h)/8)
	local x4,y4=flr((v.x+w-1)/8),flr((v.y+h)/8) -- +w-1
	local x5,y5=flr((v.x+w)/8),flr((v.y+h-1)/8) -- +w -- +h-1
	local x6,y6=flr((v.x+w)/8),flr((v.y)/8) -- +w
	local x7,y7=flr((v.x+w-1)/8),flr((v.y-1)/8) -- +w-1
	...
	local x04,y04=flr((v.x+w)/8),flr((v.y-1)/8)--右上角
	local x06,y06=flr((v.x+h)/8),flr((v.y+w)/8)--右下角
	local x08,y08=flr((v.x-1)/8),flr((v.y+h)/8)--左下角
